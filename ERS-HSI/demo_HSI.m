...
img = imread('pca_img/HSI-TrT-pca.jpg');
%load('pca_img/HSI-InP-pca.mat');
grey_img = double(rgb2gray(img));
%img = img_pca;

%// We convert the input image into a grey scale image for superpixel
%// segmentation.
%grey_img = img_pca(:, :, 1); %double(mat2gray(img_pca(:, :, 1)));
%grey_img = img;
ratio = Edge_ratio(grey_img)
...
%// Output
%//=======================================================================
%grey_img = double(rgb2gray(img(:, :, 1:3)));
